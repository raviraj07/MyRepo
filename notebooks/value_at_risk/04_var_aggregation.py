...
database = username.replace("@", "_").replace(".", "_")
...
# MAGIC %sql
# MAGIC SELECT COUNT(*) FROM ws_monte_carlo
...
# DBTITLE 1,Optimize table for faster aggregation
# MAGIC %sql
# MAGIC OPTIMIZE ws_monte_carlo ZORDER BY (country, industry)
...
# MAGIC %md
# MAGIC ## `HOMEWORK` Expected shortfall
# MAGIC 
# MAGIC We consider an investment of 1$ for each instrument here. 
# MAGIC How would you adapt above code to return [expected shortfall](https://en.wikipedia.org/wiki/Expected_shortfall) (in dollar value) instead of value at risk? 
