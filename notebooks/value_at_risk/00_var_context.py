...
# MAGIC   <img src=files/antoine.amend@databricks.com/databricks_fsi_white.png style="width: 600px">
# MAGIC </div>
...
# MAGIC 
# MAGIC <img src="/files/shared_uploads/finserv/images/value_at_risk_workflow.png" alt="logical_flow" width="600">
...
# MAGIC 
# MAGIC def generate_prices(start_price):
# MAGIC     shock = np.zeros(days)
# MAGIC     price = np.zeros(days)
# MAGIC     price[0] = start_price
# MAGIC     for i in range(1, days):
# MAGIC         shock[i] = np.random.normal(loc=mu * dt, scale=sigma * np.sqrt(dt))
# MAGIC         price[i] = max(0, price[i - 1] + shock[i] * price[i - 1])
# MAGIC     return price
# MAGIC 
# MAGIC plt.figure(figsize=(16,6))
# MAGIC for i in range(1, runs_gr):
# MAGIC     plt.plot(generate_prices(start_price))
# MAGIC 
# MAGIC plt.title('Simulated price')
# MAGIC plt.xlabel("Time")
# MAGIC plt.ylabel("Price")
# MAGIC plt.show()
