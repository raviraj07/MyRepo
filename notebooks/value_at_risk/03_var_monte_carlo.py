...
# trial_table = 'antoine_fsi.ws_monte_carlo'
...
trial_date = datetime.strptime('2021-05-10', '%Y-%m-%d')
...
# MAGIC %md
# MAGIC ## `HOMEWORK` store raw trials and predict returns as UDFs
# MAGIC Given your model packaged as `pyfunc`, one can apply prediction as a user defined function. How would you store all simulated market data and predict instruments returns through a simple SQL code? How would you store model version alongside predictions?
# MAGIC 
# MAGIC ```
# MAGIC SELECT 
# MAGIC   m.seed, 
# MAGIC   m.instrument, 
# MAGIC   predict_return(m.conditions, m.instrument)
# MAGIC FROM 
# MAGIC   monte-carlo m
# MAGIC ```
